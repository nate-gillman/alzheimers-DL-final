...
def Attention_Matrix(K, Q, use_mask=False):
    ...
    NOTE: window_size refers to input_size
	"""
	...
	K_T = tf.transpose(K, perm=[0,2,1]) # (batch_size x embedding_size x window_size_keys)
	# print(f"k.T shape: {K_T.shape}")
	score = tf.matmul(Q, K_T) # K.T --> (embedding_size x window_size x batch_size)
	# divide by sqrt of size of the key embedding
	score = tf.divide(score, tf.sqrt(tf.cast(window_size_keys, tf.float32)))
	...
	# print(f"softmax shape: {softmax_score.shape}")
...
class Atten_Head(tf.keras.layers.Layer):
	...
	def call(self, inputs_for_keys, inputs_for_values, inputs_for_queries):
		...
		K = tf.tensordot(inputs_for_keys, self.W_K, axes=1)
